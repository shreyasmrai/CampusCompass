...
def get_courses_from_snowflake():
    ...
    try:
        ...
        try:
            ...
            courses = []
            for row in rows:
                positive_review_raw = row[7] if row[7] is not None else ""
                negative_review_raw = row[8] if row[8] is not None else ""
                positive_review = [review.strip() for review in positive_review_raw.split("\n") if review.strip() != ""]
                negative_review = [review.strip() for review in negative_review_raw.split("\n") if review.strip() != ""]
                course_info = {
                    "course_id": row[0],
                    "course_name": row[1],
                    "credit_hours": row[2],
...
def process_course_data(courses):
    ...
    for course in courses:
        course_details = (
            ...
            f"Positive Review: {'; '.join(course['review']['positive'])}\n"
            f"Negative Review: {'; '.join(course['review']['negative'])}"
        )
