...
def log_query_response(query, response, file_path):
    # Get the current timestamp
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    # Create the log entry
    log_entry = f"{timestamp} - QUERY: {query} - RESPONSE: {response}\n"
...
def handle_userinput(user_question, file_path):
    # Record the timestamp when the query is sent
    query_timestamp = datetime.datetime.now().isoformat()

    # Send the query to the model and record the response
    ...
    # Extract and log the latest response
    latest_response = ""
    for i, message in enumerate(st.session_state.chat_history):
        if i % 2 == 0:
            ...
            latest_response = sanitized_content  # Update the latest response
    ...
    # Log data
    log_data = f"{query_timestamp}, Query: {user_question}, Response Timestamp: {response_timestamp}, Response: {latest_response}\n"
...
def main():
    ...
    file_path = '/Users/jomraj/Desktop/chatbot-pdf/ask-multiple-pdfs/docs/chat_logs.txt'
    if user_question:
        handle_userinput(user_question, file_path)
